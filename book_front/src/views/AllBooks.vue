    ...
    <div class="main-container">
      ...
      <div class="search-bar">
        ...
        <div class="search-field search-year">
          <input v-model="filters.startYear" type="number" placeholder="Année min" />
        </div>
        <div class="search-field search-year">
          <input v-model="filters.endYear" type="number" placeholder="Année max" />
        </div>
      </div>
      ...
      <div v-else class="books-grid">
        <div v-for="(book, i) in filteredBooks" :key="book.id" class="book-card" :style="{'--i':i}">
          <div class="card-topbar">
            ...
            <button v-if="!userIsAdmin" @click="addToFavourites(book.id)" class="fav-btn">♡</button>
          </div>
          ...
          <div class="card-avatar-row">
            ...
            <span class="edition-badge">1/1</span>
          </div>
          <div class="card-info">
          ...
          <div class="card-footer">
            ...
            <div v-if="userIsAdmin" class="card-admin-btns">
              <button @click="openEditModal(book)" class="icon-btn edit-icon-btn" title="Modifier">
              ...
              <button @click="confirmDelete(book.id)" class="icon-btn del-icon-btn" title="Supprimer">
    ...
    <div v-if="showEditModal" class="modal-backdrop" @click.self="showEditModal = false">
      <div class="modal-card">
        ...
        <div class="modal-body">
          <div class="m-field"><label>Titre</label><input v-model="selectedBook.title" type="text" placeholder="Titre" /></div>
          <div class="m-field"><label>Année</label><input v-model="selectedBook.year" type="number" placeholder="Année" /></div>
          <div class="m-field">
            <label>Auteur</label>
            <div class="sel-wrap">
              ...
              <svg class="sel-arrow" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" clip-rule="evenodd"/></svg>
            </div>
          </div>
        </div>
        <div class="modal-actions">
          <button @click="showEditModal = false" class="btn-ghost">Annuler</button>
          <button @click="handleUpdate" class="btn-main">Sauvegarder →</button>
        </div>
      </div>
    </div>

    <!-- Modal Auteur -->
    <div v-if="showAuthorModal" class="modal-backdrop" @click.self="showAuthorModal = false">
      <div class="modal-card">
        <div class="modal-header">
          <h2>Nouvel auteur</h2>
          <button class="modal-close" @click="showAuthorModal = false">✕</button>
        </div>
        <div class="modal-body">
          <div class="m-field"><label>Prénom</label><input v-model="newAuthor.prenom" placeholder="Ex: Victor" /></div>
          <div class="m-field"><label>Nom</label><input v-model="newAuthor.nom" placeholder="Ex: Hugo" /></div>
        </div>
        <div class="modal-actions">
          <button @click="showAuthorModal = false" class="btn-ghost">Annuler</button>
          <button @click="saveAuthor" class="btn-main">Enregistrer →</button>
        </div>
      </div>
    </div>
...
const newAuthor = ref({ nom: '', prenom: '' });

// Extrait le nom d'auteur peu importe la structure de l'objet (prenom/nom ou name)
const authorName = (author) => {
  if (!author) return ''
  if (author.prenom || author.nom) return `${author.prenom ?? ''} ${author.nom ?? ''}`.trim()
  // Fallback: prend toutes les valeurs string de l'objet
...
const filteredBooks = computed(() => books.value.filter(book => {
  ...
  if (filters.value.author) {
    // Cherche dans TOUTES les valeurs string de l'objet auteur — indépendant du nom de champ
    const allStrings = Object.values(book.author || {})
...
const addToFavourites = (id) => alert('Livre ajouté aux favoris !')

const saveAuthor = () => {
  authorService.create(newAuthor.value)
    .then(() => { showAuthorModal.value = false; fetchData(); alert('Auteur ajouté !') })
    .catch(() => alert("Erreur lors de l'ajout"))
}

const openEditModal = (book) => { selectedBook.value = { ...book }; showEditModal.value = true }
...
.btn-toggle { padding: 8px 16px; border-radius: 10px; border: 1.5px solid #c8dff0; background: #f0f8ff; color: #005da4; font-family: 'DM Sans', sans-serif; font-size: 0.82rem; font-weight: 600; cursor: pointer; transition: all 0.2s; }
...
.search-field input { width: 100%; background: #fff; border: 1.5px solid #d4e8f5; border-radius: 10px; padding: 9px 12px 9px 34px; font-family: 'DM Sans', sans-serif; font-size: 0.86rem; color: #0c1a2e; transition: all 0.2s; }
.search-year input { padding-left: 12px; }
.search-field input::placeholder { color: #b0c8dc; }
.search-field input:focus { outline: none; border-color: #007cc3; box-shadow: 0 0 0 3px rgba(0,124,195,0.1); }
...
.book-card { background: #fff; border: 1.5px solid #e0edf7; border-radius: 18px; overflow: hidden; display: flex; flex-direction: column; box-shadow: 0 2px 8px rgba(0,93,164,0.07), 0 8px 24px rgba(0,93,164,0.06); animation: cardIn 0.45s ease both; animation-delay: calc(var(--i) * 0.04s); transition: transform 0.25s ease, box-shadow 0.25s ease, border-color 0.25s ease; }
...
.av1 { margin-left: 0; background: linear-gradient(135deg, #bfdbfe, #93c5fd); }
...
.card-author-name { font-size: 0.75rem; color: #7a96b0; font-weight: 400; }
...
.table-img { width: 36px; height: 50px; object-fit: cover; border-radius: 6px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
...
.btn-edit-sm, .btn-del-sm { padding: 6px 13px; border-radius: 8px; border: none; font-family: 'DM Sans', sans-serif; font-size: 0.74rem; font-weight: 600; cursor: pointer; transition: all 0.2s; }
.btn-edit-sm { background: linear-gradient(135deg, #005da4, #0094d9); color: #fff; }
.btn-edit-sm:hover { transform: translateY(-1px); box-shadow: 0 4px 10px rgba(0,93,164,0.28); }
.btn-del-sm { background: #fee2e2; color: #b91c1c; }
.btn-del-sm:hover { background: #fecaca; }

.modal-backdrop { position: fixed; inset: 0; background: rgba(0,30,70,0.5); backdrop-filter: blur(8px); display: flex; align-items: center; justify-content: center; z-index: 200; padding: 24px; animation: bdIn 0.2s ease; }
@keyframes bdIn { from { opacity: 0; } to { opacity: 1; } }
.modal-card { width: 100%; max-width: 460px; background: rgba(255,255,255,0.97); border-radius: 22px; overflow: hidden; box-shadow: 0 24px 64px rgba(0,30,70,0.28); animation: mIn 0.3s cubic-bezier(0.34,1.56,0.64,1) both; }
@keyframes mIn { from { opacity: 0; transform: scale(0.92) translateY(14px); } to { opacity: 1; transform: scale(1) translateY(0); } }
...
.modal-close { width: 28px; height: 28px; border-radius: 50%; border: none; background: #f0f8ff; color: #5a80a0; font-size: 0.8rem; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s; }
.modal-close:hover { background: #dbeafe; color: #005da4; }
...
.m-field label { font-size: 0.73rem; font-weight: 700; color: #1e3a5a; letter-spacing: 0.04em; text-transform: uppercase; }
.m-field input, .m-field select { width: 100%; background: rgba(240,248,255,0.7); border: 1.5px solid #c8dff0; border-radius: 10px; padding: 10px 13px; font-family: 'DM Sans', sans-serif; font-size: 0.88rem; color: #0c1a2e; transition: all 0.2s; }
.m-field input:focus, .m-field select:focus { outline: none; border-color: #007cc3; background: #fff; box-shadow: 0 0 0 3px rgba(0,124,195,0.12); }
.sel-wrap { position: relative; }
.m-field select { appearance: none; cursor: pointer; padding-right: 32px; }
.sel-arrow { position: absolute; right: 10px; top: 50%; transform: translateY(-50%); width: 13px; color: #9ab8cf; pointer-events: none; }
.modal-actions { display: flex; justify-content: flex-end; gap: 10px; padding: 14px 26px 22px; border-top: 1px solid #e0edf7; }
.btn-ghost { padding: 9px 18px; border-radius: 10px; border: 1.5px solid #c8dff0; background: transparent; color: #5a80a0; font-family: 'DM Sans', sans-serif; font-size: 0.86rem; font-weight: 500; cursor: pointer; transition: all 0.2s; }
.btn-ghost:hover { background: #f0f8ff; }
.btn-main { padding: 9px 22px; border-radius: 10px; border: none; background: linear-gradient(135deg, #005da4, #0094d9); color: #fff; font-family: 'DM Sans', sans-serif; font-size: 0.86rem; font-weight: 600; cursor: pointer; box-shadow: 0 4px 12px rgba(0,93,164,0.3); transition: all 0.25s; }
.btn-main:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(0,93,164,0.38); }

@media (max-width: 768px) { .page { padding: 12px; } .main-container { padding: 24px 18px 36px; } .search-bar { flex-direction: column; gap: 8px; } .search-year { flex: 1; min-width: 0; } .books-grid { grid-template-columns: repeat(2, 1fr); gap: 14px; } }
